			...
			<?php 
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $currentMonthFirstDay+1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)){
						...
						// //fetched the year and month.
					 //    $emp_get_date = $_GET['_date'];
					 //    $emp_date = DateTime::createFromFormat("Y-m-d", "$emp_get_date");
						// $emp_date_year =  $emp_date->format("Y");
						// $emp_date_month =  $emp_date->format("m");

						// $displayDate = $emp_date_year.'-'.$emp_date_month.'-'.$dayCount; 
						...
						echo ($eventNum > 0)?'<a style="text-size:1px;" href="javascript:;" onclick="getEvents(\''.$currentDate.'\');">view Details</a>':'';
						echo '</div></div>';
